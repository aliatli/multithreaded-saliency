...
void SLIC::Run(const Mat original_image,
  ...
  int N = sqrt(K);
	...
	// Iterate many times
        cout << "max_iteration: " << max_iteration << "\n";
	for (size_t i = 0, max = 15; i < max; ++i) {
               // #pragma omp parallel for default(shared)
                //MTUpdateNeighbors(centroid, lab_image, labels);
		// Iterate over all centroids (one label per centroid)
            ...
            for (size_t centroid = 0, max = centroids.size(); centroid < max; ++centroid) 
                    //MTUpdateNeighbors(centroid, lab_image, labels);
                    //
                    threads.push_back(thread(&SLIC::MTUpdateNeighbors, *this , centroid, ref(lab_image), ref(labels)));
...
void SLIC::MTUpdateNeighbors(int centroid, Mat lab_image, Mat& labels){
    
    Point2i centroid_coords = centroids[centroid];
    Point2i neighbor_coords;
    Vec3f centroid_lab;
    float last_distance, actual_distance;
    Vec3f neighbor_lab;
    ...
    for (size_t i = 0, max = this->neighbors.rows; i < max; ++i) {
    	for (size_t j = 0, max = this->neighbors.cols; j < max; ++j) {
	// Get centroid's neighbors coordinates and CIELAB color
            Point2i neighbor_coords = Point2i(this->min_x + j, this->min_y + i);

...
Mat GMRsaliency::GetSup(const Mat &image)
{
	...
	SLIC slic;
        chrono::time_point< chrono::system_clock > run = chrono::system_clock::now();
	slic.Run(image, spcount, compactness, 5);
        chrono::time_point< chrono::system_clock > runEnd = chrono::system_clock::now();
        chrono::duration<double> elapsedTime = runEnd - run;
        cout << elapsedTime.count() << " seconds for SLIC" << endl;
        
        
        chrono::time_point< chrono::system_clock > get = chrono::system_clock::now();
	labels = slic.GetLabels();
        
        chrono::time_point< chrono::system_clock > endGet = chrono::system_clock::now();
        elapsedTime = endGet- get;
    
        cout << elapsedTime.count() << " seconds for GetGMR" << endl;
